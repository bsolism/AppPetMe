...
const updatePet = (pet, onUploadProgress) => {
  const data = new FormData();
  data.append("petId", pet.petId);
  data.append("name", pet.name);
  data.append("category", pet.category);
  data.append("description", pet.description);
  data.append("old", parseInt(pet.old));
  ...
  pet.file.forEach((image, index) => {
    const name = image.split("/").reverse();
    data.append("file", {
      name: name[0],
      type: "image/jpeg",
      uri: image,
  ...
  return client.put(endpoint + "/" + pet.petId, data, {
    onUploadProgress: (progress) =>
      onUploadProgress(progress.loaded / progress.total),
  });
};
export default { getPets, getPetsByHouseId, addPet, updatePet };
